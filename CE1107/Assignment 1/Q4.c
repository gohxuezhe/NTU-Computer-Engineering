...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    int frontListLength, backListLength;

    if (ll->size % 2 == 0)
    {
        frontListLength = backListLength = ll->size / 2;
    }
    else
    {
        backListLength = ll->size / 2;
        frontListLength = backListLength + 1;
    }

    for (int i = 0; i < frontListLength; i++)
    {
        insertNode(resultFrontList, i, findNode(ll, i)->item);
    }

    for (int j = 0; j < backListLength; j++)
    {
        insertNode(resultBackList, j, findNode(ll, j + frontListLength)->item);
    }
}
