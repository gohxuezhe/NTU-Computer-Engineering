...
void sumToC(LinkedList* ll, int C, ArrayList* al)
{
	// found return condition
    if (ll->sum == C){
        ArrayNode *tempNode = malloc(sizeof(ArrayNode));
		tempNode->itemArray = malloc(ll->size * sizeof(int));
        tempNode->sizeArray = ll->size;
        tempNode->next = NULL;
        ...
        for (int i = 0; i < ll->size; i++){
            tempNode->itemArray[i] = templlNode->item;
    ...
    int current = 1;
    if (ll->size != 0){
        current = findNode(*ll, ll->size-1)->item + 1;
    }
	// further explore with starting point at previous point
