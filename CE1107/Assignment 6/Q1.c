...
{
    ListNode *duplicate = HashSearch(*Q1Ptr, key);
    if (duplicate != NULL)
    {
        return 0;
    }
    ...
    {
        Q1Ptr->Table[index].head = node;
        node->pre = Q1Ptr->Table[index].head;
        Q1Ptr->Table[index].size++;
    }
    else
    {
        node->next = Q1Ptr->Table[index].head;
        node->next->pre = node;
        Q1Ptr->Table[index].head = node;
        node->pre = Q1Ptr->Table[index].head;
        Q1Ptr->Table[index].size++;
...
int HashDelete(HashTable *Q1Ptr, int key)
{
    ...
    {
        int index = Hash(key, Q1Ptr->hSize);
        if (duplicate == Q1Ptr->Table[index].head)
        {
            /* head -> duplicate <-> duplicate.next */
            Q1Ptr->Table[index].head = duplicate->next;
            duplicate->next->pre = Q1Ptr->Table[index].head;
        }
        else
        {
            if (duplicate->next == NULL)
            {
                /* duplicate.pre <-> duplicate -> NULL */
                duplicate->pre->next = NULL;
            }
            else
            {
                /* duplicate.pre <-> duplicate <-> duplicate.next */
                duplicate->next->pre = duplicate->pre;
                duplicate->pre->next = duplicate->next;
            }
        }
        Q1Ptr->Table[index].size--;
        free(duplicate);
