// import java library classes
// google "java api spec" to view available classes in JDK
// Scanner class is most commonly used class to get console input and others..(check API Spec)
import java.util.Scanner;

class P4 // class name - file name must be same as class name
{
    // // for a start, use "public static" when defining your methods (function)
    // // "public static" make the method global just like 'C'
    // // when you have learnt OO, you will use LESS of static
    // public static <return type> methodname(<argument list>) {
    // //write your code here
    // // think in 'C' syntax
    // }

    // entry into a Java program - in 'C' is int main()
    public static void main(String args[]) {
        // write your code here
        // think in 'C' syntax
        // use "System.out.println" for output display
        // "System.out.println" uses "+" to concatenate (join) String and other variable
        // data

        Scanner scanner = new Scanner(System.in); // creating an instance of Scanner class
        System.out.println("Height:"); // print question for user
        int height = scanner.nextInt(); // prompt user for height

        if (height == 0) { // error condition
            System.out.println("Error Input!!"); // reject
        } else { // accept
            for (int i = 1; i <= height; i++) { // using for loop for height of pattern
                for (int j = 1; j <= i; j++) { // using for loop for each row in pattern
                    if (i % 2 == 0) {
                        if (j % 2 == 0) { // print AA or BB based on even and odd position
                            System.out.print("AA");
                        } else {
                            System.out.print("BB");
                        }
                    } else {
                        if (j % 2 == 0) { // print AA or BB based on even and odd position
                            System.out.print("BB");
                        } else {
                            System.out.print("AA");
                        }
                    }

                }
                System.out.print("\n"); // print new line at the end of each row to force cursor to the start of the
                                        // next line
            }
        }
        scanner.close(); // close scanner to prevent leaks
    }
}